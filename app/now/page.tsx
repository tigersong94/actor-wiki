    ...
    async function fetchData() {
      try {
        ...
        const kobisRes = await fetch(
          `https://www.kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=${process.env.NEXT_PUBLIC_KOBIS_API_KEY}&targetDt=${dt}`
        )
        ...
        const [movieRes, tvRes] = await Promise.all([
          fetch(`https://api.themoviedb.org/3/trending/movie/week?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=ko-KR&region=KR`),
          fetch(`https://api.themoviedb.org/3/trending/tv/week?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=ko-KR&region=KR`)
        ])
        const movieData = await movieRes.json()
        const tvData = await tvRes.json()
        
        const combined = [
          ...(movieData.results || []).slice(0, 5).map((m: any) => ({ ...m, media_type: 'movie' })),
          ...(tvData.results || []).slice(0, 5).map((t: any) => ({ ...t, media_type: 'tv' }))
...
{boxOffice.map((movie, i) => (
  <div key={i} className="now-card">
    ...
    <div className="now-info">
      <div className="now-movie-title">{movie.movieNm.length > 15 ? movie.movieNm.slice(0, 15) + '…' : movie.movieNm}</div>
...
{ottMovies.map((movie, i) => (
  <div key={i} className="now-card">
    ...
    <div className="now-info">
      <div className="now-movie-title">{(movie.title || movie.name || '').length > 15 ? (movie.title || movie.name || '').slice(0, 15) + '…' : (movie.title || movie.name)}</div>
      <div className="now-meta">{movie.media_type === 'movie' ? '영화' : 'TV'} · ⭐ {movie.vote_average.toFixed(1)}</div>
